...
- [x] 初始化导师和实习生 Mock 账号。
- [ ] 前端封装认证 API：登录、Mock 登录、注册、当前用户。
- [ ] 前端保存 token 和用户信息到统一用户状态中。
- [ ] 前端 Axios 请求自动附加 `Authorization` header。
- [ ] 前端路由守卫拦截未登录访问。
- [ ] 前端登录页提供用户名密码表单。
- [ ] 前端登录页提供导师 Mock 登录按钮。
- [ ] 前端登录页提供实习生 Mock 登录按钮。
- [ ] 登录成功后跳转任务列表页。
- [ ] token 失效或接口返回未登录时清理本地登录状态并返回登录页。
...
- [ ] 前端刷新页面后仍能根据 token 恢复登录态。
- [ ] 前端未登录访问任务页或仪表盘时跳转登录页。
